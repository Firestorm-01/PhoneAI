...
bash# You need:
Android Studio Hedgehog or newer  (download at developer.android.com)
JDK 17+
...
bash# Unzip, open terminal inside the PhoneAI folder
./gradlew test
...
bash# 1. Enable USB debugging on phone
...
PermissionPathAccessibilitySettings → Accessibility → PhoneAI → ONNotification accessSettings → Notifications → Notification Access → PhoneAIDefault Phone appSettings → Apps → Default Apps → Phone → PhoneAIDevice AdminAuto-prompted on first launch
